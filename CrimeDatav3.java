/** Vibha belavadi
 *  Crime region definition with only three zip-code characters 
 */
...
public class CrimeDatav3 {
			...
			if(!(inp.contains("Crime ID") || inp.contains("Reported by") || inp.contains("Month") || inp.contains("Easting") || inp.contains("Northing") || inp.contains("Falls Within") || inp.contains("Location") || inp.contains("Crime Type") || inp.contains("Context"))){
				...
				if(data.length > 4){
					if((data[4].trim()).length() > 0)
						easternRegionCode = data[4].trim().substring(0, 3);
					else
						easternRegionCode = "000";
				}
				else{
					easternRegionCode = "000";
					northernRegionCode = "000";
					crimeType = "N.A";
				}
				if(data.length > 5){
					if((data[5].trim()).length() > 0)
						northernRegionCode = data[5].trim().substring(0, 3);
					else
						northernRegionCode = "000";
				}
				else{
	...
	public static void main(String[] args) throws Exception {
		...
		Job job = Job.getInstance(conf, "CrimeDatav3");
		
		job.setJarByClass(CrimeDatav3.class);
