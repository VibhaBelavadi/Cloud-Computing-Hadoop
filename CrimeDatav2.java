/** Vibha Belavadi
 *  Crime region definition with only one zip-code character
 */
...
public class CrimeDatav2 {
			...
			if(!(inp.contains("Crime ID") || inp.contains("Reported by") || inp.contains("Month") || inp.contains("Easting") || inp.contains("Northing") || inp.contains("Falls Within") || inp.contains("Location") || inp.contains("Crime Type") || inp.contains("Context"))){
				...
				if(data.length > 4){
					if((data[4].trim()).length() > 0)
						easternRegionCode = String.valueOf(data[4].trim().charAt(0));
					else
						easternRegionCode = "0";
				}
				else{
					easternRegionCode = "0";
					northernRegionCode = "0";
					crimeType = "N.A";
				}
				if(data.length > 5){
					if((data[5].trim()).length() > 0)
						northernRegionCode = String.valueOf(data[5].trim().charAt(0));
					else
						northernRegionCode = "0";
				}
				else{
	...
	public static void main(String[] args) throws Exception {
		...
		Job job = Job.getInstance(conf, "CrimeDatav2");
		
		job.setJarByClass(CrimeDatav2.class);
