/** Vibha Belavadi
 *  Crime region definition with only two zip-code characters 
 */
...
public class CrimeDatav4 {
			...
			if(!(inp.contains("Crime ID") || inp.contains("Reported by") || inp.contains("Month") || inp.contains("Easting") || inp.contains("Northing") || inp.contains("Falls Within") || inp.contains("Location") || inp.contains("Crime Type") || inp.contains("Context"))){
				...
				if(data.length > 4){
					if((data[4].trim()).length() > 0)
						easternRegionCode = data[4].trim().substring(0, 2);
					else
						easternRegionCode = "0";
				}
				else{
				...
				if(data.length > 5){
					if((data[5].trim()).length() > 0)
						northernRegionCode = data[5].trim().substring(0, 2);
					else
						northernRegionCode = "0";
				}
				else{
	...
	public static void main(String[] args) throws Exception {
		...
		Job job = Job.getInstance(conf, "CrimeDatav4");
		
		job.setJarByClass(CrimeDatav4.class);
